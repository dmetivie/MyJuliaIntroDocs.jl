...
# ╔═╡ bd5cdd9e-8fb0-11ee-2f6e-4df13c5cf39e
begin
	using DifferentialEquations # The name is pretty clear!
	using ParameterizedFunctions # extansion of DiffEq for simple ode def
	using JuMP# Optimization framework
	using HiGHS # A nonlinear solver used by JuMP
	using Measurements # Uncertainty propagation
end

# ╔═╡ 28c2712e-0c63-4d23-9a5b-6e89adc20d73
using StatsPlots

# ╔═╡ 3353c664-c4aa-48e6-9765-d2cd1c827968
begin
...
# ╔═╡ c6ed4103-41a5-469a-ab4f-13b8d89bdb44
begin
	using Turing
	using LinearAlgebra
	using Random
...
begin
    ...
    .toc-row { margin: 0 !important; padding: 1 !important; }
	main {
	    $(max_width_html)
	}
	$(md_font)
...
# ╔═╡ 7902515b-ea54-45c2-8e8f-e2f037bf0eeb
function Base.show(io::IO, ::MIME"text/html", page::WebPage)
	print(io, ShortCodes.webpage(page.url, 400*1.5,700*1.5))
end

# ╔═╡ 76b3cd0c-f5f5-409f-a0e0-365e7dbfed9d
ExtendedTableOfContents(hide_preamble = false)

# ╔═╡ 655daef8-eaaa-4ee5-b309-0af19e74b71e
click_to_present = present_button()

# ╔═╡ 8af05794-b19c-4656-a631-c604e68e6883
md"""
!!! warning 
	This notebook is heavy to load. In the Status (bottom right) at the bottom, one can make a box `notify me` when it's done
"""

# ╔═╡ 0a309431-8c38-4f60-a95c-9c787be46d62
md"""
# Is it fast?
"""

# ╔═╡ 706f1865-9ede-41a1-b2ee-73ceb2bbbd11
Resource("https://media.springernature.com/full/springer-static/image/art%3A10.1038%2Fs41592-023-01832-z/MediaObjects/41592_2023_1832_Fig3_HTML.png?as=webp")

# ╔═╡ df137b93-a029-43a2-8f43-e9cb1db1feb5
md"""
# Simple examples
"""

# ╔═╡ df776666-7d6e-483f-abe7-d5cb7b751e2f
md"""
## Lotka Volterra
"""

# ╔═╡ 4787fabf-ef50-48ac-9d6e-c75d3c46e788
lotka_volterra = @ode_def begin
    d🐭 = α*🐭 - β*🐭*🐱
    d🐱 = δ*🐭*🐱 - γ*🐱
end α β δ γ;

# ╔═╡ 69df1a0b-d3a1-4b85-92c6-fe65e965ca32
p₀ = [1.5, 1.0, 3.0, 1.0]

# ╔═╡ f38a92dc-7d09-49f3-9cbe-86d2d95a45a4
u₀ = [1.0; 1.0];

# ╔═╡ 39beccf1-599a-448d-8edd-dc0f7125349a
tspan = (0.0, 10.0)

# ╔═╡ 95acea4c-d8cf-497d-bdd3-a076746287a8
ode_prob = ODEProblem(lotka_volterra, u₀, tspan, p₀)

# ╔═╡ 4b71eabf-b91a-47d2-bde8-4775397ed2e5
sol = solve(ode_prob, Tsit5(), reltol = 1e-6, abstol = 1e-6; saveat=0.1)

# ╔═╡ 464fbb87-e75d-47fb-9824-f896e02a46e6
plot(sol)

# ╔═╡ a4c5aac0-523c-4234-9722-74c37c02d1f3
md"""
## Pendulum
"""

# ╔═╡ 9b896651-6743-4000-8110-e499f1a31501
...
# ╔═╡ 71561bfc-3672-4375-8d26-7f4e6acc9bc4
let
	...
	p_pend = (g, L)  # Numerical parameters
	prob_pend = ODEProblem(pendulum!, u₀, tspan, p_pend)
	sol_pend = solve(prob_pend, Tsit5(), reltol = 1e-6)
	plot(sol_pend.t, getindex.(sol_pend.u, 2); label=nothing)
end

# ╔═╡ 3629fc41-4ee5-4155-bbfa-c9d3bb9dfad4
md"""
## Optimization (linear or nonlinear)
"""

# ╔═╡ 34b3e64f-9705-41f5-91a8-4100373ee393
md"""
Reference ecosystem: [JuMP](https://jump.dev/)
"""

# ╔═╡ 09baed83-1a9d-4a00-ad3b-fec09cf996fc
md"""
...
# ╔═╡ 074f7692-7ae6-4e59-8937-a43878880bb6
let
	n, m = 5, 50
	M = 10
...
# ╔═╡ 128d6866-c89e-4716-81ea-1cf0d859d0cd
md"""
# Automatic Differentiation
"""

# ╔═╡ 9d881178-04c9-40af-b21d-06200f3ea084
md"""
!!! note
	The (almost) whole language is automatically differentiable. See [JuliaDiff.org](https://juliadiff.org/) for more.

	⇒ Any code, e.g., ODE, Neural Networks, ... is differentiable, hence it can be combined with optimization problems, Bayesian problems, etc.
"""

# ╔═╡ 304633c4-3603-47a0-84a8-78fd4d76754f
md"""
# Example with Bayesian ODE
"""

# ╔═╡ 71a95ea4-1ae1-4956-a6ab-75aa57e308c8
md"""
[^from]: From a tutorial of [Turing.jl](https://turing.ml/v0.22/tutorials/10-bayesian-differential-equations/)
"""

# ╔═╡ a2016bb4-695e-4c1d-bc8a-e4ff6e9ee1e0
md"""
Get noisy data from previous Lotka-Volterra
"""

# ╔═╡ 5e86db54-da6e-4732-8492-3f68cb575564
odedata = Array(sol) + 0.8 * randn(size(Array(sol)))

# ╔═╡ a04ddb4b-b3b1-4016-b2c3-f06a16e9f158
begin
...
# ╔═╡ 733e8a0a-5124-4150-82ce-4b9a8ba04a41
md"""
## Bayesian model with Turing
"""

# ╔═╡ 988ec90c-f122-4fb2-8739-62073841c4e8
@model function model_LV(data, prob)
    # Prior distributions.
    σ ~ InverseGamma(2, 3)
...
# ╔═╡ 844cc638-4496-479c-9af8-ab6858c83d82
model = model_LV(odedata, ode_prob)

# ╔═╡ d3d2f7b3-5ebb-4849-8158-00d548993f37
# Sample 3 independent chains with forward-mode automatic differentiation (the default).
chain = sample(model, NUTS(0.65), MCMCSerial(), 1000, 1; progress=false)

# ╔═╡ 5b7fea17-b3fb-4793-a3f3-e02d78f817bb
plot(chain)

# ╔═╡ 8a5b5a2c-0bdc-49cc-ab48-7e256afddd5a
md"""
## Data retrodiction
"""

# ╔═╡ 3d9c9dd9-71f4-4018-ba23-91989229f232
begin
	plot(; legend=false)
	posterior_samples = sample(chain[[:α, :β, :γ, :δ]], 300; replace=false)
	for p in eachrow(Array(posterior_samples))
	    sol_p = solve(ode_prob, Tsit5(); p=p, saveat=0.1)
...
manifest_format = "2.0"
project_hash = "d516f89e79970877cdb9d285efa698a06c981aa8"
...
git-tree-sha1 = "f555c1f8c6f75b10d31f94132d4acb4cbf0bdeef"
...
git-tree-sha1 = "611f224132a79ea476621178be37174681e7d93e"
